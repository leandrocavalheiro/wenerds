...
{
    public static class WeMethods
    {
        /// <summary>
        /// Return current datetime with DateTimeKind <br/>
        /// </summary>
        /// <param name="isUtcTime">IsUtcTime? Default: true </param>
        /// <returns>DateTimeOffset. The current DateTimeOffset</returns>
        public static DateTimeOffset Now(bool isUtcTime = true)
            => isUtcTime ? DateTimeOffset.UtcNow : DateTimeOffset.Now.LocalDateTime;
        ...
        public static WePaginatedViewModel<WeEnumViewModel> GetEnumValues(string fullNameEnum, string filter = null, int page = 1, int pageSize = 25, Dictionary<OrderEnum, OrderOrientationEnum> sortBy = null, object translationResource = null)
        {            
            if (fullNameEnum is null)
                return new WePaginatedViewModel<WeEnumViewModel>(null, page, pageSize, 0, 0);
            ...
            var enumResult = new List<WeEnumViewModel>();
            ...
            {                
                ...
                enumResult.Add(new WeEnumViewModel(value.ToString(), ((int)value).ToString(), description));
            }

            if (sortBy.Count > 0)
            {                
                var firstOrder = true;
                IOrderedEnumerable<WeEnumViewModel> enumOrdered = null;
            ...
            return new WePaginatedViewModel<WeEnumViewModel>(records, page, pageSize, total, totalPages);
        }
